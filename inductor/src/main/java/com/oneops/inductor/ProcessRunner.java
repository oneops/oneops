...
public class ProcessRunner {
  ...
  private static final int TIMEOUT_EXCEPTION_EXIT_CODE = -143;

  private static final int INTERRUPTED_EXCEPTION_EXIT_CODE = -2;
  ...
  public void executeProcess(String[] cmd, String logKey, ProcessResult result, Map<String, String> additionalEnvVars,
    ...
    try {
      ...
      executor.directory(workingDir);
      executor.timeout(timeoutInSeconds, TimeUnit.SECONDS);
      ...
      result.setResultCode(executor.execute().getExitValue());
      ...
      result.setResultCode(TIMEOUT_EXCEPTION_EXIT_CODE);
    } catch (InterruptedException e) {
      logger.error(logKey + e);
      result.setResultCode(INTERRUPTED_EXCEPTION_EXIT_CODE);
    } catch (IOException e) {
      logger.error(logKey + e);
      result.setResultCode(IO_EXCEPTION_EXIT_CODE);
    }
  }
