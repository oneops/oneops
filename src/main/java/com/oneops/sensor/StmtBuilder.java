...
public class StmtBuilder {
    ...
    public final static String STMT_RESET = "insert into OpsCloseEvent select trigger.ciId as ciId, trigger.manifestId as manifestId, trigger.name as name, trigger.source as source, reset.timestamp as timestamp, reset.metrics as metrics, 'close' as state from pattern [every trigger=OpsEvent(state = 'open') -> reset=OpsEvent(ciId = trigger.ciId and name = trigger.name and state = 'reset')]";
    public final static String STMT_RESET_HEARTBEAT = "insert into OpsCloseEvent select trigger.ciId as ciId, trigger.manifestId as manifestId, trigger.name as name, trigger.source as source, reset.timestamp as timestamp, 'close' as state from pattern [every trigger=OpsEvent(state = 'open' and type = 'heartbeat') -> reset=PerfEvent(ciId = trigger.ciId and source = trigger.source)]";
