...
/**
 * {@code DNSUtil} has some simple utility methods to deal with
 * IP address and DNS of the system.
 *
 * @author Suresh G
 */
public class DNSUtil {
    ...
    /**
     * Resolves the DNS name to IPV4 addresses using the system default DNS
     * configuration.
     *
    ...
    public static List<String> resolve(String fqdn) {
        return resolveHosts(Arrays.asList(fqdn), true, true);
    }

    /**
     * Creates a comma separated list of host addresses by resolving the hostname(s).
     * If the <b>host_resolve</b> property is <code>false</code>, returns the same host list.
     *
        ...
        if (hostResolve) {
            ...
            try {
                ...
                if (ips.contains(localIP)) {
                    logger.info("Found localhost ip (" + localIP + ") in the list. " +
