 ...
 *  
 *******************************************************************************/
package com.oneops.cms.controller.service;
...
public class CmsWoProvider {
	 ...
	 */
	//This is depricated call used by the old controller workflow for BC
	
	public List<CmsWorkOrder> getWorkOrders(long deploymentId, String state, Integer execOrder, Integer limit) {
		return null;
	}	
	/*	
		List<CmsWorkOrder> workOrders = limit != null ? dpmtMapper.getWorkOrdersLimited(deploymentId, state, execOrder, limit) 
													  : dpmtMapper.getWorkOrders(deploymentId, state, execOrder);
		List<CmsRfcCI> rfcs_ids = dpmtMapper.getDeploymentRfcCIs(deploymentId, state, execOrder);
		List<CmsRfcCI> rfcs = new ArrayList<CmsRfcCI>();
		for (CmsRfcCI rfcCi : rfcs_ids) {
			if (rfcCi.getRfcAction().equalsIgnoreCase("add")) {
				...
				CmsRfcCI rfcCimerged = cmrfcProcessor.getCiById(rfcCi.getCiId(), "df");
				rfcs.add(rfcCimerged);
			}
		}
		
		//Align rfc with deployment records
		Map<String, CmsRfcCI> rfcMap = getRfcMap(rfcs);
		for (CmsWorkOrder workOrder : workOrders) {
			if (!rfcMap.containsKey(workOrder.getRfcId())) {
				throw new DJException(CmsError.DJ_INCONSITENCY_WITH_DEPLOYMENT_RECORD_ERROR,
                        "Inconsitency with deplyment record, can't find rfc with rfcId=" + workOrder.getRfcId());
			}
			workOrder.setRfcCi(rfcMap.get(workOrder.getRfcId()));
		}	

		populateWoBase(workOrders);
		
		
		List<CmsRfcCI> envs = null;
		List<CmsRfcCI> assemblys = null;
		List<CmsRfcCI> orgs = null;
        CmsCI env = null;
		...
		for (CmsWorkOrder workOrder : workOrders) {
			if (env == null) {
				//lets populate environment and global vars just once since it's all the same for the deployment
				env = getEnv(workOrder.getRfcCi().getCiId());
				globalVars = cmsUtil.getGlobalVars(env);
			};
			// for the cloud and local vars we need to evaluate for each workorder
			Map<String, String> cloudVars = cmsUtil.getCloudVars(workOrder.getCloud());
			Map<String, String> localVars = cmsUtil.getLocalVars(workOrder.getBox());
			...
			//put env
			if (envs==null) {
				envs = getRfcCIRelatives(workOrder.getBox().getCiId(),"manifest.ComposedOf","to", null, "df");
			}
			workOrder.putPayLoadEntry("Environment", envs);
			
			//put assembly
			if (assemblys == null) {
				assemblys = getRfcCIRelatives(workOrder.getPayLoad().get("Environment").get(0),"base.RealizedIn","to", null, "df");
			}
			workOrder.putPayLoadEntry("Assembly", assemblys);
			
			//put Organization
			if (orgs == null) {
				orgs = getRfcCIRelatives(workOrder.getPayLoad().get("Assembly").get(0),"base.Manages","to", null, "df");
			}
			workOrder.putPayLoadEntry("Organization", orgs);
	...
	*/
	...
	/*
	private Map<String, CmsRfcCI> getRfcMap(List<CmsRfcCI> rfcs) {
        try {
        	Map<String, CmsRfcCI> rfcMap = new ListUtils<String>().toMap(rfcs, "rfcId");
        	return rfcMap;
		} catch (NoSuchMethodException e) {
			e.printStackTrace();
			throw new DJException(CmsError.DJ_SYSTEM_ERROR, e.getMessage());
