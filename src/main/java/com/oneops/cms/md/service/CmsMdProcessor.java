...
public class CmsMdProcessor {
    ...
    private Map<String, CmsClazz> mdClazzCache = Collections.synchronizedMap(new HashMap<String, CmsClazz>());
    private Map<Integer, CmsClazz> mdClazzCacheById = Collections.synchronizedMap(new HashMap<Integer, CmsClazz>());
    private Map<String, CmsRelation> mdRelationCache = Collections.synchronizedMap(new HashMap<String, CmsRelation>());
    private Map<Integer, CmsRelation> mdRelationCacheById = Collections.synchronizedMap(new HashMap<Integer, CmsRelation>());
    private boolean cacheEnabled = false;
    
    
    public static int BASE_CLASS_ID = 100;

	static Logger logger = Logger.getLogger(CmsMdProcessor.class);

	private final AttrComparator attrComparator = new AttrComparator();	

	static {
		logger.warn(">+initalizing+ CmsMdProcessor");
	}
    ...
    private List<CmsClazzAction> getAllClazzActions(CmsClazz clazz, boolean isSuperClass ) {
        ...
        List<CmsClazzAction> thisActions = null;
        if (isSuperClass) {
        ...
        Map<String,CmsClazzAction> actionsMap = new HashMap<String, CmsClazzAction>();
        ...
        return new ArrayList<CmsClazzAction>(actionsMap.values());
    }
	...
	*/
    ...
    public void deleteClazz(int clazzId, boolean deleteAll) {
        logger.debug("Dlete class id = "+clazzId);
    ...
    private List<CmsClazzRelation> parseTargets(CmsClazzRelation link) {
    	...
    	List<CmsClazzRelation> targets = new ArrayList<CmsClazzRelation>();
    ...
    private List<CmsClazz> getClassesByPattern(String pattern){
    	List<CmsClazz> classes = new ArrayList<CmsClazz>();
    	if (pattern == null) {
    		return classes;
    	}
    	if (pattern.endsWith("*")) {
