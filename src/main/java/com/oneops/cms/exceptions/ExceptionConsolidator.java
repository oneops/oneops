...
public class ExceptionConsolidator {
    ...
    public ExceptionConsolidator(Class<? extends CmsBaseException> exception, int errorCode, int countOfErrorsToReport) {
    ...
    public void invokeChecked(Runnable r) {
        try {
            ...
            if (exception.isInstance(e)) {
                if (hasErrorCountExceeded()) rethrowExceptionIfNeeded();
    ...
    public void rethrowExceptionIfNeeded() {
        if (!errors.isEmpty()) {
            try {
                throw exception.getDeclaredConstructor(Integer.TYPE, String.class).newInstance(errorCode, StringUtils.join(
                        getErrorsToReport(), ";\n"));
