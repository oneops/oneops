...
public class CmsCmRfcMrgProcessor {
    ...
    private static final Set<String> DUMMY_RELS = initSet("manifest.WatchedBy");
	...
	private CmsRfcCI requestCiDeleteCascade(long ciId,  String userId, Map<String, CmsClazzRelation> targets, int execOrder) {
		...
		for (CmsRfcRelation rel : fromRels) {
			if (rel.getToRfcCi() != null) {
				if (targets == null) targets = new HashMap<String, CmsClazzRelation>();
			...
			//if this is cross layer relation (like manifest to bom) do not submit an rfc
			// since it will generate another release in bom namesapce
			// it will gets deleted when master release gets complete
	...
	private List<CmsRfcCI> filterRfcCis(List<CmsRfcCI> rfcs, List<AttrQueryCondition> attrConds) {
		List<CmsRfcCI> resultList = new ArrayList<CmsRfcCI>();
	...
	private List<CmsRfcCI> mergeRfcCiList(Collection<Cis> cisList, String cmAttrValue) {
		List<CmsRfcCI> rfcCis = new ArrayList<CmsRfcCI>();
		for (Cis cis: cisList) {
	...
	private Map<Long, Cis> buildCiMap(List<CmsCI> ciList, List<CmsRfcCI> rfcList) {
		Map<Long, Cis> cisMap = new HashMap<Long, Cis>();
		for (CmsCI ci : ciList) {
			Cis cis = new Cis();
			cis.setCi(ci);
	...
	private Map<Long, Relations> buildRelationsMap(List<CmsCIRelation> cmRelations, List<CmsRfcRelation> rfcRelations) {
		Map<Long, Relations> relsMap = new HashMap<Long, Relations>();
	...
	private List<CmsRfcRelation> mergeRelations(
		...
		List<CmsRfcRelation> resultRels = new ArrayList<CmsRfcRelation>();
	...
	public void populateRelCis(List<CmsRfcRelation> rels, boolean fromCis, boolean toCis, String cmAttrValue) {
		...
		if (toCis) {
			...
			Map<Long, CmsRfcCI> ciMap = new HashMap<Long, CmsRfcCI>();
			for (CmsRfcCI rfc : getCiByIdList(new ArrayList<Long>(toCiIds), cmAttrValue)) {
		...
		if (fromCis) {
			...
			Map<Long, CmsRfcCI> ciMap = new HashMap<Long, CmsRfcCI>();
			for (CmsRfcCI ci : getCiByIdList(new ArrayList<Long>(fromCiIds), cmAttrValue)) {
...
/*
	...
	*/
	...
	public List<CmsLink> getLinks(String nsPath, String relName) {
		...
		Map<String, CmsLink> mergedMap = new HashMap<String, CmsLink>();
		...
		return new ArrayList<CmsLink>(mergedMap.values());
	}
