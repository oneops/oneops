if node.workorder.payLoad.has_key?('EscortedBy') &&
  ...
  attachments.each do |a|
    a[:ciAttributes][:run_on].split(",").each do |r|
      before.push(a) if ["before-#{node.workorder.rfcCi.rfcAction}"].index(r)
    end
  end
  
  before.sort_by { |a| a[:ciAttributes][:priority] }.each do |a|
    
    Chef::Log.info("Loading before-#{node.workorder.rfcCi.rfcAction} attachment #{a[:ciName]}") 
    ...
    if a[:ciAttributes].has_key?("exec_cmd")
      ...
      ruby_block "execute command for after-#{node.workorder.rfcCi.rfcAction} #{a[:ciName]} attachment" do
        block do
