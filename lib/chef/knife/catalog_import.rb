...
class Chef
  class Knife
    class CatalogImport < Chef::Knife

      banner "knife catalog import CATALOG (options)"
      ...
      option :reload,
        :long => "--reload",
        :description => "Remove the current design before importing"
      ...
      def run
        ...
        if config[:all]
          ui.info("Starting import of all catalogs")
          Chef::Catalog.upload_all
          ui.info("Completed import of all catalogs!")
        else
          if @name_args.empty?
            ui.error "You must specify the catalog name to import or use the --all option."
            exit 1
          end
          @name_args.each do |name|
            ui.info("Starting import of catalog #{name}")
            catalog = Chef::Catalog.from_disk(name)
            catalog.description(config[:desc])
            Chef::Log.debug(catalog.inspect)
            if catalog.public != "true"
              ui.info("Catalog #{name} not marked for public import. Please add 'public: true' to enable imports to public namespace!")
              exit 1
            end
            ...
            ui.info("Completed import of catalog #{name}!")
          end
        end
