...
public class OpsProcedureProcessor {
   ...
   * @return the cms ops procedure
   */
  public CmsOpsProcedure processProcedureRequest(CmsOpsProcedure procRequest,
      OpsProcedureDefinition procDef) {
    List<CmsOpsAction> actions = getProcedureActions(procDef, procRequest.getCiId(),
        procRequest.getArglist());
  ...
  private List<CmsOpsAction> getProcedureActions(OpsProcedureDefinition procDef, long anchorCiId,
      String argList) {
  ...
  private List<CmsOpsAction> processBranch(long anchorCiId, OpsProcedureFlow flow, int offset) {
    ...
    if (flow.getNsPath() != null && flow.getNsPath().trim().length() > 0) {
      ...
      String fullNsPath = anchorCiNsPath.endsWith("/") ? anchorCiNsPath + flow.getNsPath()
          : anchorCiNsPath + "/" + flow.getNsPath();
      ...
      if (flow.getTargetIds() != null && flow.getTargetIds().size() > 0) {
        links = cmProcessor.getFromCIRelationsByToCiIdsNaked(anchorCiId, flow.getRelationName(),
            flow.getRelationShortName(), flow.getTargetIds());
      } else {
        links = cmProcessor.getFromCIRelationsNaked(anchorCiId, flow.getRelationName(),
            flow.getRelationShortName(), flow.getTargetClassName());
      }
    } else {
      if (flow.getTargetIds() != null && flow.getTargetIds().size() > 0) {
        links = cmProcessor.getToCIRelationsByFromCiIdsNaked(anchorCiId, flow.getRelationName(),
            flow.getRelationShortName(), flow.getTargetIds());
      } else {
        links = cmProcessor
            .getToCIRelationsNaked(anchorCiId, flow.getRelationName(), flow.getRelationShortName(),
                flow.getTargetClassName());
      }
    }
  ...
  private OpsException createExceptionForAnchorBlock(long blockeeCiId, String blockeeName,
      CmsOpsProcedure blockingProcedure) {
    StringBuilder sb = new StringBuilder(GIVEN_CI);///String for front end error msg
    sb.append(blockeeCiId).append(WITH_PROCEDURE_ACTION_NAME).append(blockeeName)
        .append(ALREADY_HAS_ACTIVE_OPS_PROCEDURE).append(blockingProcedure.getCiId())
        .append(PROCEDURE_NAME).append(blockingProcedure.getProcedureName()).append(CURRENT_STATUS)
        .append(blockingProcedure.getProcedureState().name());

    return new OpsException(CmsError.OPS_ALREADY_HAVE_ACTIVE_PROCEDURE_ERROR, sb.toString())
        .set(OpsException.ExceptionDetailKey.BLOCKING_PROCEDURE_CI_ID.name(),
///fill details Map in exception...
  ...
  private OpsException createExceptionForActionBlock(long blockeeCiId, String blockeeName,
      CmsOpsProcedure blockingProcedure) {
    StringBuilder sb = new StringBuilder(GIVEN_CI);///String for front end error msg
    sb.append(blockeeCiId).append(WITH_PROCEDURE_ACTION_NAME).append(blockeeName)
        .append(ALREADY_HAS_ACTIVE_OPS_ACTION).append(blockingProcedure.getCiId())
        .append(PROCEDURE_NAME).append(blockingProcedure.getProcedureName()).append(CURRENT_STATUS)
        .append(blockingProcedure.getProcedureState().name());

    return new OpsException(CmsError.OPS_ALREADY_HAVE_ACTIVE_ACTION_ERROR, sb.toString())
        .set(OpsException.ExceptionDetailKey.BLOCKING_PROCEDURE_CI_ID.name(),
///fill details Map in exception...
            String.valueOf(blockingProcedure.getCiId()))
        .set(OpsException.ExceptionDetailKey.PROCEDURE_ID.name(),
  ...
  /**
   * Creates the cms ops procedure. Can throw an OpsException if the proc argument lacks an action,
   * or if there exists already a conflicting procedure
   *
   * @param proc the proc
   * @return the cms ops procedure
   */
  public CmsOpsProcedure createCmsOpsProcedure(CmsOpsProcedure proc) {
    ...
    if (proc.getActions().isEmpty()) {
      logger.warn("throwing exception, CmsOpsProcedure contains zero actions for proc ciId:" + proc
          .getCiId());
    ...
    if (blockingProcedure != null) {
      logger
          .warn("throwing exception, procedure blocked by action: " + blockingProcedure.getCiId());
    ...
    for (CmsOpsAction action : proc.getActions()) {
      ...
      if (blockingProcedure != null) {
        logger.warn("throwing exception, procedure's action blocked by action: " + blockingProcedure
            .getCiId());
  ...
  /**
   * Checks for active or pending ops procedures that have an ops action associated with the input
   * proc's CiId
   *
   ...
   * proc's CiId
   *
   * @param action CmsOpsAction to be checked
   * @return CmsOpsProcedure that has such action, or null if none fit criteria
   */
  ...
  private CmsOpsProcedure getActionHoldingOpsProcedureForCi(long ciId) {
    ...
    List<CmsOpsProcedure> cmsOpsProcedures = opsMapper
        .getProcedureForCiByAction(ciId, null, null, null);
    ...
    if (cmsOpsProcedures != null && cmsOpsProcedures.size() > 0) {
      for (CmsOpsProcedure anOpsProcedure : cmsOpsProcedures) {
        if (anOpsProcedure.getProcedureState().equals(OpsProcedureState.pending) || anOpsProcedure
            .getProcedureState().equals(OpsProcedureState.active)) {
  ...
  /**
   * checks for any active or pending procedures that block execution of input CmsOpsProcedure
   * argument. If any such blockers exist, one of them will be returned otherwise if no such
   * blocking exists the method returns null
   *
  ...
  /**
   * checks for any active or pending procedures in the system that block execution of input
   * CmsOpsAction argument. If any such blockers exist, one of them will be returned otherwise if no
   * such blocking exists the method returns null
   *
   * @return CmsOpsProcedure which blocks, or else null if no blocking
   */
   ...
   */
  public List<CmsOpsProcedure> getCmsOpsProceduresForCi(long ciId,
      List<OpsProcedureState> stateList, String procedureName, Integer limit) {
    List<CmsOpsProcedure> procedures = opsMapper
        .getProcedureForCi(ciId, stateList, procedureName, limit);
   ...
   * @return the cms ops procedures for ci
   */
  public List<CmsOpsProcedure> getCmsOpsProceduresForCiByAction(long ciId,
      List<OpsProcedureState> stateList, String procedureName, Integer limit) {
    List<CmsOpsProcedure> procedures = opsMapper
        .getProcedureForCiByAction(ciId, stateList, procedureName, limit);
    for (CmsOpsProcedure procedure : procedures) {
   ...
   */
  public List<CmsOpsProcedure> getCmsOpsProceduresForNamespace(String nsPath,
      List<OpsProcedureState> stateList, String procedureName) {
    List<CmsOpsProcedure> procedures = opsMapper
        .getProcedureForNamespace(nsPath, stateList, procedureName);
  ...
  public List<CmsOpsProcedure> getCmsOpsProcedureForNamespaceLike(String nsPath,
    ...
    List<CmsOpsProcedure> procedures = opsMapper
        .getProcedureForNamespaceLike(nsPath, nsLike, stateList, procedureName, limit);
  ...
  public CmsOpsProcedure updateProcedureState(long procId, OpsProcedureState state) {
    ...
    proc.setProcedureState(state);
    handleProcedureComplete(proc);
    return opsMapper.getCmsOpsProcedure(procId);
  }

  private void handleProcedureComplete(CmsOpsProcedure proc) {
    if (OpsProcedureState.complete == proc.getProcedureState()) {
      performCustomUpdates(proc);
    }
  }

  private void performCustomUpdates(CmsOpsProcedure proc) {
    if (GSLB_MIGRATION.equals(proc.getProcedureName()) && StringUtils.isNotBlank(proc.getArglist())) {
      MigrateArg migrateArg = gson.fromJson(proc.getArglist(), MigrateArg.class);

      if (StringUtils.isNotBlank(migrateArg.getMigrate())) {
        String migrateTo = migrateArg.getMigrate().toLowerCase();
        if (GSLB_TYPE_NETSCALER.equals(migrateTo) || GSLB_TYPE_TORBIT.equals(migrateTo)) {
          logger.info("gslb-migration custom updates procedure_id : " + proc.getProcedureCiId());

          List<CmsOpsAction> actions = getCmsOpsActions(proc.getProcedureId());
          if (!actions.isEmpty()) {
            CmsOpsAction action = actions.get(0);
            CmsCI ci = cmProcessor.getCiById(action.getCiId());

            if (ci.getCiClassName().matches(BOM_FQDN)) {
              String serviceType = ciAttribute(ci, SERVICE_TYPE_ATTR);

              //update manifest attribute only when the bom attribute matches the procedure attribute value
              if (migrateTo.equals(serviceType)) {
                List<CmsCIRelation> relations = cmProcessor
                    .getToCIRelations(ci.getCiId(), BASE_REALIZED_AS, null, null);
                if (relations != null && !relations.isEmpty()) {
                  CmsCI manifestCi = relations.get(0).getFromCi();
                  logger.info("gslb-migration updating manifest ciId : " + manifestCi.getCiId()
                      + " to service_type " + migrateTo);
                  CmsCIAttribute serviceTypeAttr = manifestCi.getAttribute(SERVICE_TYPE_ATTR);
                  serviceTypeAttr.setDfValue(migrateTo);
                  serviceTypeAttr.setDjValue(migrateTo);
                  serviceTypeAttr.setOwner("manifest");
                  cmProcessor.updateCI(manifestCi);
                }
              } else {
                logger.info("gslb-migration : bom fqdn service_type [" + serviceType
                    + "] does not match with migrate procedure argument [" + migrateTo + "]");
              }
            }
          }
        }
  ...
  private String ciAttribute(CmsCI ci, String attribute) {
    CmsCIAttribute attr = ci.getAttribute(attribute);
    if (attr == null) {
      return "";
    }
    return attr.getDjValue();
  }
  ...
  public CmsOpsAction updateOpsAction(CmsOpsAction action) {
    ...
    if (existingAction == null) {
      throw new OpsException(CmsError.OPS_PROCEDURE_NOT_FOUND_ERROR,
          NO_ACTION_ITH_ID + action.getActionId());
    }
    if ((action.getActionState().equals(OpsActionState.complete) || action.getActionState()
        .equals(OpsActionState.failed)) && !existingAction.getActionState()
        .equals(OpsActionState.inprogress)) {
      throw new OpsException(CmsError.OPS_ACTION_IS_NOT_IN_PROGRESS,
          ACTION_IS_IN_WRONG_STATE + action.getActionId() + "; old state " + existingAction
              .getActionState() + "; new state " + action.getActionState());
    }
  ...
  public long getCmsOpsProceduresCountForCiFromTime(long ciId, List<OpsProcedureState> stateList,
    ...
    return opsMapper
        .getCmsOpsProceduresCountForCiFromTime(ciId, stateList, procedureName, timestamp);
  }
